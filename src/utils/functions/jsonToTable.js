...
module.exports = function transformJSONToTable(docs, options = {}) {
  ...
  let allHeaders = _.keys(
    traverse(docs).reduce(function (headers, value) {
      const self = this;
      ...
      if (this.isLeaf) {
        this.path = _.map(this.path, (level) => {
          if (level.indexOf(".") > -1 && self.level > 2) {
            // If a leaf contains a dot in it, then surround the whole path with ticks
            level = `\`${level}\``;
          }
          return level;
        });
        if (!(_.isPlainObject(value) && _.keys(value).length === 0)) {
          // Check against empty objects. Don't treat these paths as a valid header value.
          headers[_.tail(this.path).join(".")] = true;
        }
      }
      return headers;
  ...
  tableData = tableData.concat(
    _.map(docs, (doc) => {
      return _.map(allHeaders, (header) => {
        ...
        if (header.indexOf(".`") > -1) {
          // One of those special cases where a path is nested, AND has a dot in the name.
