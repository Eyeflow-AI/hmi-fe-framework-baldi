...
export default function WordingTable({ availableLanguages, usedLanguages }) {
  const { t } = useTranslation();
  ...
  useEffect(() => {
    if (usedLanguages?.languageList?.length > 0) {
      ...
      let columns = [];
      let defaultLanguage = usedLanguages.languageList.find(
        (language) => language.id === defaultLanguageId
      );
      let otherLanguages = usedLanguages.languageList.filter(
        (language) => language.id !== defaultLanguageId && language.active
      );
