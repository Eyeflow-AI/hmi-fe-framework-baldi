...
const getAnnotatedImg = ({
  ...
  img.onload = function () {
    ...
    if (options.severalAnnotations) {
      ...
      (Array.isArray(regions) && regions.length > 0 ? regions : []).forEach((region, i) => {
        // if (bboxRegion) {
          // console.log({ bboxRegion })
...
export default function ListView({
  ...
  const drawImage = async ({ url, index, sizes, regions, isSelectedSerialRunning=false, imageInfo }) => {
    ...
    if (absolute_path) {
      ...
      _url = `${url}/${absolute_path}/${imageInfo?.image_file ?? imageInfo?.image_file}`;
    }
    else {
      _url = `${url}/eyeflow_data/event_image/${imageInfo?.image_path ?? imageInfo?.image_path}/${imageInfo?.image_file ?? imageInfo?.image_file}`;
    }
  ...
  useEffect(() => {
    ...
    if (inspections.length === 1) {
      let detections = [...inspections[0]?.event_data?.inspection_result?.detections];
          ...
          // dataToUse.map((inspection, index) => (
            <Box
              sx={{
                ...
                backgroundColor: dataToUse?.inspection?.result ? `${colors.statuses['ok']}50` : `${colors.statuses['ng']}50`,
              ...
              >
                ...
                >
                  <Typography textAlign={'center'} textTransform={'uppercase'}>
                    ...
                    <span
                      style={{
                        color: dataToUse?.inspection?.result ? colors.green : colors.red,
                        fontWeight: 'bold',
                      }}
                    >
